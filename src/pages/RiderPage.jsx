...
const [to, setTo] = useState("");
    ...
    <Layout title="Rider Dashboard">
      <div className="grid gap-6">
        <Card
          ...
          right={<Button onClick={createRide} disabled={busy}>{busy ? "Requesting..." : "Request Ride"}</Button>}
        >
  ...
  />
        ...
        <Card title="My rides" right={<Button variant="secondary" onClick={refresh}>Refresh</Button>}>
          {rides.length === 0 ? (
                  ...
                  {rides.map((r) => (
                    <tr key={r.id} className="border-b last:border-b-0">
                      ...
                      <td className="py-3 pr-3">
                        <button className="text-xs underline text-slate-700 hover:text-slate-900" onClick={() => navigator.clipboard.writeText(r.id)}>
                          Copy ID
                        </button>
                      </td>
                    </tr>
